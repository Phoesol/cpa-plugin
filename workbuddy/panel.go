...
func buildDashboardEx(force, fetchCredits bool) map[string]any {
	...
	if force && lifecycleEnabled() {
		...
		if files2, err2 := hostAuthList(); err2 == nil {
			...
			// Use a fresh slice — out[:0] would alias the backing array we're
			// ranging over, which is a footgun if anyone later mutates out.
			filtered := make([]wbAccount, 0, len(out))
			for _, a := range out {
